...
// TODO: 
// Option to append or replace items in updateItems()?
// Option to NOT select first item in updateItems()?
...
; (function ($, window, undefined) {
    ...
    select.prototype.initialise = function () {
        ...
        if (this.url != undefined && this.display.text().length > 0) {
            ...
            var id = this.values.filter(function () {
                return $(this).attr('name').toLowerCase() === idProperty;
            }).val();
            this.fetchItems(searchText, id);
        }
        if (this.selectedItem === undefined) {
            // Disable the value to prevent postback
    ...
    select.prototype.updateItems = function (data, id) {
        // TODO: Make it an option to append or replace
    ...
    select.prototype.fetchItems = function (searchText, id) {
        var self = this;
        ...
        if (self.cache[searchText] === undefined) {
            $.getJSON(url, function (data) {
                ...
                self.updateItems(self.cache[searchText], id);
            });
        }
        else {
            self.updateItems(self.cache[searchText], id);
